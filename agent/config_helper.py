...
def extract_agent_config(agent_data: Dict[str, Any], version_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    is_aabc_default = metadata.get('is_aabc_default', False)
    ...
    if version_data:
        ...
        if is_aabc_default:
            ...
            agentpress_tools = AABCConfig.DEFAULT_TOOLS
        ...
        config = {
            ...
            'is_aabc_default': is_aabc_default,
    ...
    if agent_data.get('config'):
        ...
        if is_aabc_default:
            from agent.aabc.config import AABCConfig
            config['system_prompt'] = AABCConfig.get_system_prompt()
            config['tools']['agentpress'] = AABCConfig.DEFAULT_TOOLS
...
def build_unified_config(
    ...
    aabc_metadata: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
    simplified_tools = {}
    ...
    if aabc_metadata:
        config['aabc_metadata'] = aabc_metadata
...
def is_aabc_default_agent(config: Dict[str, Any]) -> bool:
    return config.get('is_aabc_default', False)
...
def can_edit_field(config: Dict[str, Any], field_name: str) -> bool:
    if not is_aabc_default_agent(config):
        return True
...
def get_default_system_prompt_for_aabc_agent() -> str:
    from agent.aabc.config import AABCConfig
    return AABCConfig.get_system_prompt()
