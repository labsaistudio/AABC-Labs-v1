...
from .config_manager import AABCConfigManager, AABCConfiguration
from .repository import AABCAgentRepository, AABCAgentRecord
...
class AABCSyncService:
    def __init__(self):
        self.config_manager = AABCConfigManager()
        self.repository = AABCAgentRepository()

    async def sync_all_agents(self, dry_run: bool = False) -> SyncResult:
        logger.info("🚀 Starting AABC agent metadata sync")

        try:
            current_config = self.config_manager.get_current_config()
            agents_needing_sync = await self.repository.find_aabc_agents_needing_sync(
                current_config.version_tag
            )

            if not agents_needing_sync:
                logger.info("📋 All AABC agents already have current metadata")
                return SyncResult(
                    success=True,
                    synced_count=0,
    ...
    async def install_for_all_missing_users(self) -> SyncResult:
        logger.info("🚀 Installing AABC agents for users who don't have them")

        try:
            current_config = self.config_manager.get_current_config()
            all_accounts = await self.repository.get_all_personal_accounts()
            existing_agents = await self.repository.find_all_aabc_agents()
            ...
            if not missing_accounts:
                return SyncResult(
                    success=True,
                    details=[{"message": "All users already have AABC agents"}]
                )

            logger.info(f"📦 Installing AABC for {len(missing_accounts)} users")
            ...
            for account_id in missing_accounts:
                try:
                    await self.repository.create_aabc_agent_simple(
                        account_id,
                        current_config.version_tag
                    )
                    success_count += 1
                    logger.info(f"✅ Installed AABC for user {account_id}")
                except Exception as e:
                    failed_count += 1
    ...
    async def get_sync_status(self) -> Dict[str, Any]:
        try:
            ...
            return {
                ...
                "note": "System prompt & tools always current from AABCConfig"
            }
