...
class AABCAgentRecord:
    ...
    @classmethod
    def from_db_row(cls, row: Dict[str, Any]) -> 'AABCAgentRecord':
...
class AABCAgentRepository:
    def __init__(self, db: DBConnection = None):
        self.db = db or DBConnection()

    async def find_all_aabc_agents(self) -> List[AABCAgentRecord]:
        try:
            client = await self.db.client
            result = await client.table('agents').select(
                'agent_id, account_id, name, metadata'
            ).eq('metadata->>is_aabc_default', 'true').execute()

            return [AABCAgentRecord.from_db_row(row) for row in result.data]

        except Exception as e:
            logger.error(f"Failed to find AABC agents: {e}")
            raise

    async def find_aabc_agents_needing_sync(self, target_version_tag: str) -> List[AABCAgentRecord]:
        agents = await self.find_all_aabc_agents()
        return [
    ...
    async def create_aabc_agent_simple(
        self,
        account_id: str,
        version_tag: str
    ) -> str:
        try:
            ...
            agent_data = {
                ...
                "is_default": True,
                "system_prompt": "[AABC_MANAGED]",
                "agentpress_tools": {},
                "configured_mcps": AABCConfig.DEFAULT_MCPS,
                "custom_mcps": AABCConfig.DEFAULT_CUSTOM_MCPS,
                "config": {
                    'tools': {
                        'mcp': AABCConfig.DEFAULT_MCPS,
                        'custom_mcp': AABCConfig.DEFAULT_CUSTOM_MCPS,
                        'agentpress': {}
                    },
                    'metadata': {
                        'is_aabc_default': True,
            ...
            if result.data:
                ...
                logger.info(f"Created minimal AABC agent {agent_id} for {account_id}")
                return agent_id

            raise Exception("No data returned from insert")

        except Exception as e:
            logger.error(f"Failed to create AABC agent for {account_id}: {e}")
            raise
    ...
    async def update_agent_metadata(
        ...
        """Update only metadata for a AABC agent (system prompt/tools read from AABCConfig)"""
        try:
            client = await self.db.client
