...
class AABCConfig:
    NAME = "AABC Agent"
    DESCRIPTION = "AABC Agent is your blockchain AI assistant from AABC Labs, equipped with Web3 tools and smart contract capabilities to help you build, deploy, and manage crypto projects."
    AVATAR = "🤖"
    AVATAR_COLOR = "#3B82F6"
...
def add_tool(tool_name: str, description: str, enabled: bool = True):
    AABCConfig.DEFAULT_TOOLS[tool_name] = {
        "enabled": enabled,
        "description": description
    }

def remove_tool(tool_name: str):
    if tool_name in AABCConfig.DEFAULT_TOOLS:
        del AABCConfig.DEFAULT_TOOLS[tool_name]

def enable_tool(tool_name: str):
    if tool_name in AABCConfig.DEFAULT_TOOLS:
        AABCConfig.DEFAULT_TOOLS[tool_name]["enabled"] = True

def disable_tool(tool_name: str):
    if tool_name in AABCConfig.DEFAULT_TOOLS:
        AABCConfig.DEFAULT_TOOLS[tool_name]["enabled"] = False
