...
from agent.tools.blockchain_tools import (
    SolanaBalanceTool,
    SolanaTransferTool,
    SolanaSwapTool,
    SolanaBinksTool,
...
class ToolManager:
    ...
    def register_all_tools(self):
        ...
        # Register AABC Labs Blockchain Tools
        logger.info("Registering AABC Labs blockchain tools...")
        self.thread_manager.add_tool(SolanaBalanceTool, thread_manager=self.thread_manager)
        self.thread_manager.add_tool(SolanaTransferTool, thread_manager=self.thread_manager)
        self.thread_manager.add_tool(SolanaSwapTool, thread_manager=self.thread_manager)
        self.thread_manager.add_tool(SolanaBinksTool, thread_manager=self.thread_manager)
        self.thread_manager.add_tool(SolanaTokenTool, thread_manager=self.thread_manager)
        logger.info("✅ Blockchain tools registered successfully")
...
class AgentRunner:
    ...
    async def setup_tools(self):
        ...
        if self.config.agent_config and self.config.agent_config.get('is_aabc_default', False):
            aabc_agent_id = self.config.agent_config['agent_id']
            tool_manager.register_agent_builder_tools(aabc_agent_id)
        ...
        if self.config.agent_config and 'agentpress_tools' in self.config.agent_config:
            ...
            if isinstance(raw_tools, dict):
                if self.config.agent_config.get('is_aabc_default', False) and not raw_tools:
                    enabled_tools = None
                else:
